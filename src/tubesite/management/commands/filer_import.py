...
class Command(BaseCommand):
    ...
    Import directory structure into the filer ::
        ...
        manage.py filer_import --path=<point_to_your_local_image_folder> --folder=<folder_path_in_filer>
        manage.py filer_import --folder=a --storage=storages.backends.ftp.FTPStorage public_html
        manage.py filer_import --path=/tmp/assets/news --duplicate-action=update images content something
    """
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--duplicate-action',
            ...
            dest='dup_action',
            default=None,
            help='Specify constanct action when duplicate file is found in Filer (one of %s)' % list(DUPLICATE_ACTIONS))
    ...
    def handle(self, *paths, **options):
        ...
        file_importer.import_path(options['path'])
